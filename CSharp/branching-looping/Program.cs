...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "2":
            ...
            {
                Console.WriteLine(
                    $"We currently have {petCount} pets thatneed homes. We can manage {(maxPets - petCount)} more."
                );
            }
        ...
        case "3":
            for (int i = 0; i < maxPets; i++)
            {
                ...
                // Skip over any animal when petID is set to the default value
                if (animalID == "ID #: ")
                {
                    continue;
                }
                ...
                if (validEntry == false)
                {
                    do
                    {
                        ...
                        if (readResult != null)
                        {
                            validEntry = int.TryParse(readResult, out int enteredPetAge);
                            if (validEntry == true)
                            {
                                ourAnimals[i, 2] = string.Concat("Age: ", Convert.ToString(enteredPetAge));
                            }
                        }
                    } while (validEntry == false);
                }
